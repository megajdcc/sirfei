...
class TipoServicio{
	...
	function ListarCatalogo(){
		...
		$sentenciaSQL = "SELECT nombre,id from tiposervicio";
	...
	function Verificar($nombre){
		...
		$sql = "SELECT * FROM tiposervicio where nombre = :nombre";
	...
	function registrar($nombre){
		...
		$sql = "INSERT INTO tiposervicio (nombre) values(:nombre)";
	...
	function capturardatos($id){
		try {
			$this->conexion->beginTransaction();
			$sql = "select * from tiposervicio where id =:id";
			$stm = $this->conexion->prepare($sql);
			$stm->bindParam(':id',$id,PDO::PARAM_INT);
			$result = $stm->execute();
	...
	function modificar($nombre,$id){
		try {
			$result = false;
			$this->conexion->beginTransaction();
			$sql = "UPDATE tiposervicio SET nombre = :nombre where id =:id";
			$stm = $this->conexion->prepare($sql);
			$stm->bindParam(':nombre',$nombre,PDO::PARAM_STR);
			$stm->bindParam(':id', $id,PDO::PARAM_INT);
			$result = $stm->execute();
